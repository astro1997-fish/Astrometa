...
const depositAddresses_1 = require("./depositAddresses");
// Minimal ABI for the ERC-20 Transfer event — used to watch USDT/USDC
exports.ERC20_TRANSFER_ABI = [
...
const ETH_PRICE_STALE_THRESHOLD_MS = parseInt(process.env.ETH_PRICE_STALE_THRESHOLD_MS ?? String(15 * 60 * 1000), 10);
/**
...
auditOverride) {
    ...
    try {
        await (0, pushNotifications_1.sendDepositConfirmedPush)(userId, amountUsd, 'eth');
    }
    catch (e) {
    ...
    if (auditOverride) {
        try {
            const { data: admin } = await supabase_1.supabase
                ...
                .single();
            await email_1.emailService.sendAdminOverrideAlert({
                amountUsd,
 ...
 */
async function verifyEvmTransaction(coin, txHash, expectedRaw) {
    const rpcUrl = process.env.ETH_RPC_URL;
    if (!rpcUrl)
        return { error: 'ETH_RPC_URL not configured' };
    const addressSets = await (0, depositAddresses_1.loadActiveEvmAddressSets)();
    ...
    if (!receipt)
        return { error: 'Transaction not found on chain — check the hash' };
    if (receipt.status !== 1)
        return { error: 'Transaction reverted on chain — cannot credit' };
    if (coin === 'eth') {
        ...
        if (tx.value !== expectedRaw) {
            return { error: `Amount mismatch — this transaction sent ${ethers_1.ethers.formatEther(tx.value)} ETH, but this deposit requires exactly ${ethers_1.ethers.formatEther(expectedRaw)} ETH. Wrong transaction hash?` };
        }
        const usdValue = await getUsdValue('ETH', tx.value, 18);
        if (usdValue === null || usdValue <= 0)
            return { error: 'ETH price unavailable or returned $0 — use manual amountUsd override instead' };
    ...
    const iface = new ethers_1.ethers.Interface(exports.ERC20_TRANSFER_ABI);
    const tokenInfo = Object.entries(exports.TOKEN_MAP).find(([, info]) => info.symbol.toLowerCase() === coin);
    if (!tokenInfo)
        return { error: `Unknown token for coin ${coin}` };
    ...
    for (const log of receipt.logs) {
        ...
        try {
            ...
            if (desc && desc.name === 'Transfer' && activeAddresses.has(desc.args[1].toLowerCase())) {
                sawTransferToActiveAddress = true;
                const rawValue = desc.args[2];
                if (rawValue !== expectedRaw)
                    continue; // amount doesn't match this deposit — keep looking, then fall through to error
                const usdValue = Number(rawValue) / 1e6;
                if (usdValue <= 0)
                    return { error: `${coin.toUpperCase()} transfer amount is $0` };
...
const listenerState = {
    ...
    lastRetryRunAt: null, // Date.now() of the last retryPendingPriceTransactions run
 ...
 */
async function maybeSendUnhealthyAlert(reason) {
    const now = Date.now();
    ...
    const details = `Current status: ${status.message}\n` +
    ...
    try {
        await email_1.emailService.sendListenerAlert(reason, details);
...
function getListenerStatus() {
    ...
    const cooldownRemainingSec = listenerState.circuitState === 'open' && listenerState.circuitOpenedAt !== null
...
const CIRCUIT_OPEN_COOLDOWN_MS = parseInt(process.env.CIRCUIT_OPEN_COOLDOWN_MS ?? String(5 * 60 * 1000), 10);
/**
...
function closeCircuit() {
    if (listenerState.circuitState !== 'closed') {
        console.log(`[Blockchain] Circuit breaker CLOSED — provider recovered after ${listenerState.consecutiveFailures} consecutive failure(s)`);
    }
...
function recordCircuitFailure() {
    ...
    if (listenerState.circuitState === 'closed' && listenerState.consecutiveFailures >= CIRCUIT_FAILURE_THRESHOLD) {
        listenerState.circuitState = 'open';
        listenerState.circuitOpenedAt = Date.now();
        console.error(`[Blockchain] Circuit breaker OPEN after ${listenerState.consecutiveFailures} consecutive failures — ` +
 ...
 */
async function matchPendingEvmDeposit(coin, rawValue) {
    const { data, error } = await supabase_1.supabase
        .from('transactions')
...
async function deferEvmDepositToPendingPrice(txId, meta) {
    const { error } = await supabase_1.supabase
        .from('transactions')
        .update({
        ...
        .eq('status', 'pending');
    if (error) {
        console.error(`[Blockchain] CRITICAL: could not defer tx ${txId} to pending_price — ` +
...
async function processNativeCandidate(provider, txHash, rawValue) {
    const eventKey = `${txHash}:native`;
    console.log(`[Blockchain] Candidate ETH deposit tx=${txHash} amount=${ethers_1.ethers.formatEther(rawValue)} — waiting ${MIN_CONFIRMATIONS} confirmations…`);
    ...
    if (!txRecord) {
        console.warn(`[Blockchain] No pending ETH deposit matches amount ${ethers_1.ethers.formatEther(rawValue)} (tx ${txHash})`);
        return;
    }
...
async function handleNewBlock(provider, blockNumber) {
    markProviderAlive();
    const addressSets = await (0, depositAddresses_1.loadActiveEvmAddressSets)();
    if (addressSets.eth.size === 0)
        return; // ETH deposits not active
    ...
    for (const tx of block.prefetchedTransactions) {
        if (!tx.to || tx.value === 0n)
            continue;
        if (!addressSets.eth.has(tx.to.toLowerCase()))
            continue;
        listenerState.lastEventAt = Date.now();
        processNativeCandidate(provider, tx.hash, tx.value).catch(err => console.error(`[Blockchain] Error processing native ETH candidate ${tx.hash}:`, err));
    }
}
/** Processes a candidate USDT/USDC Transfer event to the configured deposit address. */
async function handleTokenTransfer(coin, to, rawValue, event) {
    markProviderAlive();
    const addressSets = await (0, depositAddresses_1.loadActiveEvmAddressSets)();
    if (!addressSets[coin].has(to.toLowerCase()))
        return;
 ...
 */
async function attachDepositWatchers(provider) {
    if (activeProvider && activeProvider !== provider) {
        try {
            activeProvider.removeAllListeners();
        }
        catch { /* old provider may already be dead */ }
    }
    ...
    provider.on('block', (blockNumber) => {
        handleNewBlock(provider, blockNumber).catch(err => console.error(`[Blockchain] Error handling block ${blockNumber}:`, err));
    });
    ...
    for (const [tokenAddress, info] of Object.entries(exports.TOKEN_MAP)) {
        const coin = info.symbol.toLowerCase();
        const addresses = addressSets[coin];
        boundTokenAddressSets[coin] = new Set(addresses);
        if (addresses.size === 0)
            continue; // no active address for this coin — don't subscribe at all
        const contract = new ethers_1.ethers.Contract(tokenAddress, exports.ERC20_TRANSFER_ABI, provider);
        const filter = contract.filters.Transfer(null, Array.from(addresses));
        contract.on(filter, (_from, to, value, event) => {
            handleTokenTransfer(coin, to, value, event).catch(err => console.error(`[Blockchain] Error handling ${coin.toUpperCase()} Transfer event:`, err));
        });
 ...
 */
async function maybeRebindDepositWatchers(provider) {
    const addressSets = await (0, depositAddresses_1.loadActiveEvmAddressSets)();
    const changed = (Object.keys(exports.TOKEN_MAP).length > 0) && ['usdt', 'usdc'].some(coin => !sameAddressSet(boundTokenAddressSets[coin], addressSets[coin]));
    if (!changed)
        return;
 ...
 */
async function reconnectWithBackoff(rpcUrl) {
    ...
    try {
        ...
        await maybeSendUnhealthyAlert(`Automatic reconnection failed after ${MAX_RECONNECT_ATTEMPTS} attempts — RPC ${rpcUrl}`);
    }
    finally {
...
async function runListenerHealthCheck(rpcUrl) {
    const now = Date.now();
    ...
    if (providerOk) {
        ...
        if (activeProvider) {
            maybeRebindDepositWatchers(activeProvider).catch(err => console.error('[Blockchain] Failed to rebind deposit watchers:', err));
        }
        return;
    }
...
function startListenerHealthCheck(rpcUrl) {
    // Run once shortly after startup, then on the regular interval
    setTimeout(() => runListenerHealthCheck(rpcUrl), 30000);
    setInterval(() => runListenerHealthCheck(rpcUrl), LISTENER_HEALTH_CHECK_INTERVAL_MS);
...
let retryLoopIntervalHandle = null;
...
function startPendingPriceRetryWatchdog() {
    setInterval(() => {
        const now = Date.now();
        ...
        console.warn(`[Blockchain] Pending-price retry loop watchdog: no run detected in ` +
    ...
    console.log(`[Blockchain] Pending-price retry loop watchdog scheduled (interval: ${Math.round(RETRY_WATCHDOG_INTERVAL_MS / 60000)} min, ` +
