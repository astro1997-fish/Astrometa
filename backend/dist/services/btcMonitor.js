...
exports.loadBtcPriceCacheFromDb = loadBtcPriceCacheFromDb;
...
async function getXpub() {
    ...
    try {
        const { data } = await supabase_1.supabase
            ...
            .eq('key', 'btc_xpub')
            .maybeSingle();
        cachedXpub = data?.value ? (0, encryption_1.decryptSetting)(data.value) : null;
    }
    catch (e) {
...
async function fetchAddressTxs(address) {
    ...
    const res = await axios.get(`${BLOCKSTREAM_API}/address/${address}/txs`, { timeout: 10000 });
    return res.data;
}
async function fetchBlockchainHeight() {
    const { default: axios } = await Promise.resolve().then(() => __importStar(require('axios')));
    const res = await axios.get(`${BLOCKSTREAM_API}/blocks/tip/height`, { timeout: 10000 });
    return res.data;
}
/** Satoshi → BTC */
 ...
 */
async function persistBtcPriceCache(price, fetchedAt) {
    try {
        ...
        console.warn('[BTC] Failed to persist price cache to DB:', err.message);
    }
}
/**
 ...
 */
async function loadBtcPriceCacheFromDb() {
    try {
        ...
        console.warn('[BTC] Failed to parse persisted price cache:', err.message);
    }
}
...
async function getBtcUsdPrice() {
    ...
    try {
        ...
        if (price > 0) {
            ...
            persistBtcPriceCache(price, now).catch(() => { });
        }
...
async function atomicCreditBtc(txId, userId, amountUsd, btcTxid, fromStatuses = ['pending']) {
    ...
    if (balErr) {
        ...
        await supabase_1.supabase.from('transactions').update({ status: 'pending' }).eq('id', txId);
    ...
    try {
        await (0, pushNotifications_1.sendDepositConfirmedPush)(userId, amountUsd, 'btc');
    }
    catch (e) {
...
async function loadPendingBtcDeposits() {
    const { data, error } = await supabase_1.supabase
        .from('transactions')
 ...
 */
async function persistConfirmationProgress(depositId, txid, confirmations) {
    try {
        const { error } = await supabase_1.supabase
            .from('transactions')
            .update({
        ...
        console.warn(`[BTC] Failed to persist confirmation progress for ${depositId}:`, err.message);
    }
}
async function pollDeposit(deposit, chainHeight, btcPrice) {
    let txs;
    try {
    ...
    for (const tx of txs) {
        ...
        if (confirmations < MIN_CONFIRMATIONS) {
            ...
            try {
                ...
                console.warn(`[BTC] Failed to persist confirmation-wait reason for ${deposit.id}:`, err.message);
            }
            continue;
        }
        ...
        if (btcPrice === null) {
            ...
            console.warn(`[BTC] ${deposit.btcAddress}: tx ${tx.txid} confirmed (${btcAmount} BTC) but BTC/USD price unavailable — marking pending_price for retry`);
        ...
        if (usdValue <= 0) {
            ...
            try {
                ...
                console.warn(`[BTC] Failed to persist $0-price reason for ${deposit.id}:`, err.message);
            }
            continue;
        }
