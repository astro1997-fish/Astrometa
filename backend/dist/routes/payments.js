...
const depositAddresses_1 = require("../services/depositAddresses");
...
router.get('/capabilities', async (_req, res, next) => {
    try {
        const [xpub, ethAddr, usdtAddr, usdcAddr] = await Promise.all([
            (0, btcMonitor_1.getXpub)(),
            (0, depositAddresses_1.getActiveEvmAddress)('eth'),
            (0, depositAddresses_1.getActiveEvmAddress)('usdt'),
            (0, depositAddresses_1.getActiveEvmAddress)('usdc'),
        ]);
        res.json({
            btc: !!xpub,
            eth: !!ethAddr,
...
router.post('/create-crypto-deposit', auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const evmCoin = coin;
        const address = await (0, depositAddresses_1.getActiveEvmAddress)(evmCoin);
        if (!address) {
        ...
        const baseRaw = ethers_1.ethers.parseUnits(baseCryptoAmount.toFixed(decimals), decimals);
        const maxJitterRawNum = Math.max(1000, Math.floor((MAX_JITTER_USD / pricePerUnit) * 10 ** decimals));
        let txRecord = null;
        let matchAmountStr = null;
        let requiredRaw = null;
        for (let attempt = 0; attempt < 20; attempt++) {
            ...
            const candidateStr = ethers_1.ethers.formatUnits(candidateRaw, decimals);
            const { data, error: insertErr } = await supabase_1.supabase
                .from('transactions')
                .insert({
                user_id: userId,
...
router.get('/deposits', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
...
router.delete('/deposits/:id', auth_1.requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
